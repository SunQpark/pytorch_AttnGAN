...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, label) in enumerate(self.data_loader):
            ...
            output_0, embedding_0 = self.model.D(fake_x_0, cond)
            output_1, embedding_1 = self.model.D(fake_x_1, cond)
            output_2, embedding_2 = self.model.D(fake_x_2, cond)
            
            errG = self.loss(output_0, real_label, mu, std) + self.loss(output_1, real_label, mu, std) + self.loss(output_2, real_label, mu, std) \
                    + 0.1 *(self.pullaway_loss(embedding_0) + self.pullaway_loss(embedding_1) + self.pullaway_loss(embedding_2))
            ...
            real_recon_0, _ = self.model.D(data[0], cond)
            real_recon_1, _ = self.model.D(data[1], cond)
            real_recon_2, _ = self.model.D(data[2], cond)

            fake_x_0, fake_x_1, fake_x_2, cond, mu, std = self.model.G(label)
            errD_real = self.loss(real_recon_0, data[0]) + self.loss(real_recon_1, data[1]) + self.loss(real_recon_2, data[2])
            errD_real.backward(retain_graph=True)            
            # errD_fake = self.loss(output_0, fake_label) + self.loss(output_1, fake_label) 
            errD_fake = self.loss(output_0, fake_x_0) + self.loss(output_1, fake_x_1) + self.loss(output_2, fake_x_2)
